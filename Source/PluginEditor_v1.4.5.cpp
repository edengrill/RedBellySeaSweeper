    ...
    : AudioProcessorEditor (&p), audioProcessor (p)
{
    ...
    // Grab keyboard focus for VST3 compatibility
    grabKeyboardFocus();
}
...
void SwoopDeviceAudioProcessorEditor::resized()
{
    ...
    keyboardTooltip.setBounds(250, 110, 200, 20);
...
void SwoopDeviceAudioProcessorEditor::parentHierarchyChanged()
{
    // Grab keyboard focus when the plugin window becomes visible (important for VST3)
    if (isShowing())
        grabKeyboardFocus();
}