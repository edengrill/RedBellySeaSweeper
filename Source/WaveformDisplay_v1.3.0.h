...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // Draw Wave and Mode selectors on LEFT side
        g.setColour(juce::Colour(0xffd83427));
        g.setFont(13.0f);
        
        // Wave selector (clickable area)
        auto waveArea = juce::Rectangle<float>(bounds.getX() + 15, bounds.getBottom() - 35, 100, 15);
        juce::String waveText = "WAVE: " + waveTypes[currentWaveIndex];
        
        // Highlight on hover
        if (waveHover)
        {
            g.setColour(juce::Colour(0xffd83427).brighter());
            g.drawRoundedRectangle(waveArea.expanded(2), 2.0f, 1.0f);
        }
        g.setColour(juce::Colour(0xffd83427));
        g.drawText(waveText, waveArea, juce::Justification::left, false);
        
        // Mode selector (clickable area)
        auto modeArea = juce::Rectangle<float>(bounds.getX() + 15, bounds.getBottom() - 18, 100, 15);
        ...
        {
            g.setColour(juce::Colour(0xffd83427).brighter());
            g.drawRoundedRectangle(modeArea.expanded(2), 2.0f, 1.0f);
        }
        g.setColour(juce::Colour(0xffd83427));
